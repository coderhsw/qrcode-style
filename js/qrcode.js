...
(function () {
	...
	QRCode_Art = function (qrcode, options) {
		if (!qrcode || !options) {
		...
		if (list.row2col3.length > 0) {
			...
			for (var i = 0; i < row2col3Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['row2col3'].length
				) {
		...
		if (list.row3col2.length > 0) {
			...
			for (var i = 0; i < row3col2Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['row3col2'].length
				) {
		...
		if (list.row4.length > 0) {
			...
			for (var i = 0; i < row4Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['row4'].length
				) {
		...
		if (list.row2col2.length > 0) {
			...
			for (var i = 0; i < row2col2Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['row2col2'].length
				) {
		...
		if (list.corner.length > 0) {
			...
			for (var i = 0; i < cornerLen; i++) {
				while (
					...
					arrIndex < this.arrayObj['corner'].length
				) {
		...
		if (list.row3.length > 0) {
			...
			for (var i = 0; i < row3Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['row3'].length
				) {
		...
		if (list.col2.length > 0) {
			...
			for (var i = 0; i < col2Len; i++) {
				while (
					...
					arrIndex < this.arrayObj['col2'].length
				) {
		...
		if (list.single.length > 0) {
			...
			for (var i = 0; i < singleLen; i++) {
				while (
					...
					arrIndex < this.arrayObj['single'].length
				) {
		...
		for (var row = 0; row < this.nCount; row++) {
			for (var col = 0; col < this.nCount; col++) {
				if (isDraw[row][col]) {
					if (
						(row == 0 && col == 0) ||
						(row + 7 == this.nCount && col == 0) ||
						(row == 0 && col + 7 == this.nCount)
					) {
						// 码眼占7个码字
						for (var i = 0; i < 7; i++) {
							for (var j = 0; j < 7; j++) {
